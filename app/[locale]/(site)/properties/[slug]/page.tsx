...
import { getPropertyBySlug, agents as allAgents } from "@/lib/data";
...
export async function generateMetadata({  params  }: any) {
  ...
  return {
    title: `${title} | ${dict.meta.propertiesTitle}`,
    description:
      getLocalizedText(property.description, locale) ||
      dict.meta.propertiesDescription
  };
}
...
export default async function PropertyDetailsPage({  params  }: any) {
  ...
  const { formatArea } = useUnitMeasurement();
  ...
  const description =
    getLocalizedText(property.description, locale) ||
    getLocalizedText(property.description, "en");
        ...
        <aside className="space-y-4">
          ...
          <div className="bg-white rounded-xl shadow-sm p-5 text-sm">
            ...
            <ul className="space-y-1 text-xs text-slate-700">
              ...
              {property.furnishing && <li>Furnishing: {property.furnishing}</li>}
